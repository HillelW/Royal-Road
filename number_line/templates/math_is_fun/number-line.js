function numberlineMain(mode) {
    ...
    s +=
      ...
      "px; min-height:" +
      h +
      'px; border: none; border-radius: 20px; background-color: #def; margin:auto; display:block;">';
    s +=
      '<canvas id="canvasId" style="position: absolute; width:' +
      w +
      "px; height:" +
      h +
      'px; left: 0; top:; border: none;"></canvas>';
    s +=
      '<input id="clearBtn" onclick="clearMarks()" type="button" style="z-index:2; position:absolute; left:5px; bottom:5px;" value="clear"  class="togglebtn" />';
    s +=
      '<div id="copyrt" style="font: 10px Arial; font-weight: bold; color: #6600cc; position:absolute; right:5px; bottom:3px;"></div>';
    s += "</div>";
    document.write(s);
  ...
  function drawNumLine(x, y, wd, currX) {
    ...
    g.stroke();
    g.fillStyle = "blue";
    g.drawArrow(x + wd / 2 + 20, y, 20, 2, 30, 15, 0);
    g.fill();
    g.strokeStyle = "red";
    g.lineWidth = 2;
    g.beginPath();
    ...
    g.stroke();
    g.fillStyle = "red";
    g.drawArrow(x - wd / 2 - 20, y, 20, 2, 30, 15, Math.PI);
    g.fill();
    g.textAlign = "center";
    g.lineWidth = 1;
    g.fillStyle = "black";
    ...
    g.fillStyle = "blue";
    for (var i = 1; i <= 10; i++) {
      var xp = x + i * wd * this.scale;
      g.fillText(i.toString(), xp, y + 35);
      g.beginPath();
      g.moveTo(xp, y - 10);
      g.lineTo(xp, y + 15);
      g.stroke();
    ...
    g.fillStyle = "red";
    for (i = 1; i <= 10; i++) {
      xp = x - i * wd * this.scale;
      g.fillText("-" + i.toString(), xp - 2, y + 35);
      g.beginPath();
      g.moveTo(xp, y - 10);
      g.lineTo(xp, y + 15);
      g.stroke();
  ...
  function val2x(currX) {
    if (this.mode == "int") {
      ...
      m = (currX - this.midX) / (this.scale * this.width);
      m = m.toFixed(7);
      frac = m % 1;
