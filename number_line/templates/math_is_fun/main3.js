...
function doChina() {
  ...
  if (zhQ) {
    ...
    if (transQ) {
      ...
      var tgtDiv = document.getElementById("content");
      ...
      var str =
        ...
        path +
        '" style="color:red; text-decoration: none;">ä¸­æ–‡</a></div>';
...
function getFlash6HTML(w, h, fn, querystring, clr) {
  ...
  var s =
    '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,79,0" width="' +
    w +
    '" height="' +
    h +
    '" id="' +
  ...
  s +=
    'swLiveConnect="true" allowscriptaccess="sameDomain" allowFullScreen="true" id="' +
    fn +
    '" name="' +
    fn +
...
function putFlash6(w, h, fn, querystring, clr, noflash) {
  ...
  if (hasFlash()) {
    ...
    if (noflash) {
      ...
      s =
        '<a href="/flash-player.html"><img src="/images/style/no-flash.jpg" alt="Needs Flash Player"></a>';
    }
    document.write(s);
    if (Math.random() < 0.1) {
      var path = location.pathname;
...
function sethideadscookie(val) {
  var date = new Date();
  if (val == 1) date.setTime(date.getTime() + 1 * 36 * 60 * 60 * 1000);
...
function setAds(stateNo) {
  for (var i = 0; i < adIDs.length; i++) {
    var div = document.getElementById(adIDs[i][0]);
    if (div) {
      var styleNo = adIDs[i][stateNo + 1];
      switch (styleNo) {
        case 0:
...
function hideAds() {
  if (
    confirm(
      "This will set a Cookie so we can remember not to show ads on each page. It lasts for about a day. Do you agree?"
    )
  ) {
    sethideadscookie(1);
    setAds(1);
...
function getMenu(typ) {
  ...
  if (typ == 2) {
    s += "&nbsp;";
    s +=
      '<a href="javascript:showMenu()" style="text-decoration: none;" aria-label="Show Menu">';
    s += '<img src="' + urlStt + 'images/style/menu.svg" alt="Menu" />';
    s += "</a>";
    s += " ";
    s +=
      '<a href="javascript:showSrch()" style="text-decoration: none;" aria-label="Show Search">';
    s += '<img src="' + urlStt + 'images/style/search.gif" alt="Search" />';
    s += "</a>";
  }
  return s;
}
...
function getSearch() {
  var s = "";
  s += '<form action="../search/search.php" method="get">';
  s +=
    '<input type="text" name="query" value="" placeholder="Search" id="searchFld" aria-label="Search" />';
  s +=
    '<input type="submit" name="submit" value="" id="searchBtn" aria-label="Search Button" />';
...
function getAdRight2() {
  ...
  if (true) {
    s +=
      '<div id="google_translate_element" style="border: none; display:inline; float:left; "></div>';
    s += '<script type="text/javascript">';
    s += "	function googvarranslateElementInit() {";
    s +=
    ...
    if (true) {
      s +=
        '	<script type="text/javascript" src="//translate.google.com/translate_a/element.js?cb=googvarranslateElementInit"></script>';
    } else {
      console.log("async translate");
      s += 'var googvarranslateScript = document.createElement("script");';
      s += 'googvarranslateScript.type = "text/javascript";';
      s += "googvarranslateScript.async = true;";
      s +=
        'googvarranslateScript.src = "//translate.google.com/translate_a/element.js?cb=googvarranslateElementInit";';
      s +=
        '( document.getElementsByTagName("head")[0] || document.getElementsByTagName("body")[0] ).appendChild(googvarranslateScript);';
    }
  }
  return s;
}
...
function cookieMsg() {
  var s = "";
  s +=
    '<div id="cookieok" style="position:absolute; right:5px; top:5px; border-radius:10px; background-color:#def; padding:3px 8px; font: 14px Arial;">';
  var dom = document.domain;
  if (dom == "localhost") dom = "localhost/mathsisfun";
  s += 'We may use <a href="http://' + dom + '/about-ads.html">Cookies</a> ';
  s +=
    '<div style="display:inline-block; border-radius:10px; background-color:#abf; padding:3px 8px; cursor: pointer;" onclick="cookieok()">OK</div>';
  s += "</div>";
  return s;
}
function cookieok() {
...
function getAdCode(src) {
  ...
  switch (src) {
    case "media":
      ...
      s += " </script>";
      s +=
        ' <script src="//contextual.media.net/dmedianet.js?cid=8CU6C33V2" async="async"></script>';
      var szNum = 0;
      ...
      var szs = [
        { sz: "728x90", ht: 90, id: "496278264" },
        { sz: "468x60", ht: 60, id: "322386457" },
        { sz: "320x90", ht: 90, id: "385483814" }
      ];
      var sz = szs[szNum];
    ...
    case "mathtutor":
      ...
      s +=
        '<a href="http://www.mathtutor.com/index-ad.html"><img src="/images/ad-mathtutor.gif"></a>';
      s += " </div>";
      break;
    case "dummy":
    ...
    case "adman":
      s +=
        ' <script async="async" src="https://www.googvaragservices.com/tag/js/gpt.js"></script>';
      s += "\n";
      s += " <script>";
      s += " var googvarag = googvarag || {};";
      s += " googvarag.cmd = googvarag.cmd || [];";
      s += " </script>";
      s += "\n";
      s += " <script>";
      s += " googvarag.cmd.push(function() {";
      s +=
        ' googvarag.defineSlot("/21621894/top728x90", [[120, 60], [320, 50], [234, 60], [468, 60], [120, 90], [728, 90]], "div-gpt-ad-1551592987345-0").addService(googvarag.pubads());';
      s += " googvarag.pubads().enableSingleRequest();";
      s += " googvarag.enableServices();";
      s += " });";
      s += " </script>";
      s += "\n";
      ...
      s += " <script>";
      s +=
        ' googvarag.cmd.push(function() { googvarag.display("div-gpt-ad-1551592987345-0"); });';
      s += " </script>";
      s += " </div>";
      break;
    case "pixfuture1":
      ...
      s +=
        '<div id="' +
        id +
        '_1" style="width:320px;height:60px;margin:0;padding:0"></div>';
      s += ' <script type="text/javascript">';
      s += " var OX_ads = OX_ads || [];";
      s += " OX_ads.push({";
      s += ' slot_id: "' + id + '_1",';
      ...
      s += " </script>";
      s +=
        ' <script type="text/javascript" src="https://pixfuture-d.openx.net/w/1.0/jstag"></script>';
      break;
    case "pixfuture2":
      var id = mobQ ? "540635834" : "540635829";
      s +=
        '<div id="' +
        id +
        '_2" style="width:320px;height:60px;margin:0;padding:0"></div>';
      s += ' <script type="text/javascript">';
      s += " var OX_ads = OX_ads || [];";
      s += " OX_ads.push({";
      s += ' slot_id: "' + id + '_2",';
    ...
    case "chitika":
      ...
      s +=
        ' var unit = {"calltype":"async[2]","publisher":"mathsisfun","width":320,"height":50,"sid":"Chitika Default"};';
      s += " var placement_id = window.CHITIKA.units.length;";
      s += " window.CHITIKA.units.push(unit);";
      s +=
        " document.write('<div id=" +
        '"' +
        "chitikaAdBlock-' + placement_id + '" +
        '"' +
        "></div>');";
      s += " }());";
      s += " </script>";
    ...
    case "patreon":
      s +=
        '<a href="https://www.patreon.com/mathisfun">Support Math is Fun on Patreon</a>';
      break;
    case "kofi":
      s +=
        '<a href="https://ko-fi.com/mathisfun">Buy a Coffee for Math is Fun</a>';
      break;
    case "infolinks":
      s +=
        '<script> var infolinks_pid = 3166388; var infolinks_wsid = 0; </script> <script src="//resources.infolinks.com/js/infolinks_main.js"></script><input type="hidden" name="IL_IN_ARTICLE">';
      break;
    case "media":
      break;
    case "adsenseold":
...
function getSocial() {
  ...
  s +=
    ...
    "&t=" +
    title +
    '" title="Share on Facebook" target="_blank"><img src="/mathsisfun/images/style/Facebook.png"></a></li>';
  s +=
    '<li><a href="https://twitter.com/intent/tweet?source=' +
    url +
    "&text=" +
    title +
    ":%20" +
    url +
    '" target="_blank" title="Tweet"><img src="/mathsisfun/images/style/Twitter.png"></a></li>';
  s +=
    '<li><a href="http://pinterest.com/pin/create/button/?url=' +
    url +
    "&description=" +
    title +
    '" target="_blank" title="Pin it"><img src="/mathsisfun/images/style/Pinterest.png"></a></li>';
  s += "</ul>";
  return s;
}
function getLinks() {
  ...
  var s = "";
  s +=
    '<a href="javascript:linkToUs()" id="linkus" title="Link To Us">Link To Us</a>';
  s +=
    linkstt +
  ...
  s +=
    ...
    "&summary=" +
    title +
    '&source=mathsisfun.com" title="LinkedIn" id="linkli">LinkedIn</a>';
  s +=
    '<a href="javascript:tellAFriend()" id="linkem" title="eMail">eMail a Friend</a>';
  return s;
}
function getFooter() {
  var s = "";
  ...
  s +=
    ...
    sep +
    '<a href="../links/index.html">Index</a>' +
    sep +
    '<a href="../aboutmathsisfun.html">About</a>' +
    sep +
    '<a href="../contact.php">Contact</a>' +
    sep +
    '<a href="javascript:Citation()">Cite&nbsp;This&nbsp;Page</a>' +
    sep +
    '<b><a href="../Privacy.htm">Privacy</a></b>';
  s += "</div>";
  return s;
}
function getBodyEnd() {
...
function addView(pg, viewtype, hostname) {
  ...
  if (hostname == "localhost") {
    req.open("POST", "http://localhost/mathopolis/links/update.php", true);
  } else {
    req.open("POST", "https://www.mathsisfun.com/links/update.php", true);
  }
...
function getRelated() {
  ...
  if (left != null) {
    s +=
      '<div style="position: fixed; top: 0px; right: 50%; margin: 10px ' +
      wd +
      'px 10px 0; text-align: left; ">';
    s += fmtMenuBox(left.href, left.text, 0);
    s += "</div>";
  }
  if (right != null) {
    s +=
      '<div style="position: fixed; top: 0px; left: 50%; margin: 10px 0 10px ' +
      wd +
      'px; text-align: right; ">';
...
function fmtMenuBox(url, txt, dirn) {
  ...
  s +=
    '<a href="' +
    url +
    '"  style="text-decoration: none; color: #888888; vertical-align: middle; display: table;">';
  s += '<div id="' + boxID + '" >';
  s += txt;
  s += "</div>";
  s += "</a>";
...
function initVideo(id, titleid, spanid, style) {
  ...
  switch (style) {
    case "h1":
      ...
      s +=
        '    <img src="/images/style/video2.gif" alt="Video" width="75" height="33" style="vertical-align:middle; border:none;" />';
      s += " </a>";
      s += "</div>";
      s +=
...
function doVideo(id, spanid) {
  ...
  var visQ = visIndex > -1 ? true : false;
  if (visQ) {
